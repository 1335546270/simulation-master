...
import com.iqilu.config.local.MemberLocal;
import com.iqilu.config.local.MemberTokenMsg;
import com.iqilu.controller.BaseController;
...
//        }
        ...
        if (StringUtils.isNotBlank(token)) {
            ...
            MemberTokenMsg userTokenMsg = JSON.parseObject(String.valueOf(decode), MemberTokenMsg.class);
            // 兼容openId
            if(userTokenMsg.getId() == null && userTokenMsg.getOpenid() != null){
                String memberId = ClientAesDecrypt.aesDecode(userTokenMsg.getOpenid());
                userTokenMsg.setId(Integer.valueOf(memberId));
            }
            MemberLocal.set(userTokenMsg);
        } else {
            MemberTokenMsg userTokenMsg = new MemberTokenMsg();
        ...
        /* token验证 */
        return this.verifyToken(response, token, super.clientSecret, reqUrl);
    }
